...
from src.modelling.ctabgan.model.synthesizer import transformer
...
class CTABGANSynthesizer:
    ...
    def __init__(
        ...
        logging.info(f"device type: {self.device}")
    ...
    def fit(self, train_data=pd.DataFrame(), categorical=[], mixed={}, pb_type={}):
        ...
        if pb_type:
            problem_type = list(pb_type.keys())[0]
            if problem_type:
                target_index = train_data.columns.get_loc(pb_type[problem_type])
        ...
        optimizer_params = dict(
            lr=self.lr, betas=(0.5, 0.9), eps=1e-3, weight_decay=self.l2scale
        )
        ...
        self.Gtransformer = transformer.ImageTransformer(self.gside)
        self.Dtransformer = transformer.ImageTransformer(self.dside)
        ...
        for i in tqdm(range(self.epochs)):
            for _ in range(steps_per_epoch):
                ...
                raw_loss_g = -(torch.log(y_fake + 1e-4).mean())
                ...
                loss_g =  raw_loss_g + cross_entropy
                ...
                if problem_type:
                    ...
                    loss_cg.backward()
                    loss_total_g = loss_g + loss_info + loss_cg
                    optimizerG.step()

            # print(f'loss_g type : {type(loss_g)}')
            # print(f'loss_g type : {type(loss_g)}')        
            # print(f'loss_mean type : {type(loss_mean)}')
            # print(f'loss_mean : {loss_mean}')
            # print(f'loss_std type : {type(loss_std)}')
            # print(f'loss_info type : {type(loss_info)}')
            # print(f'loss_info : {loss_info}')
            # print(f'loss_cc : {type(loss_cc)}')
            # print(f'loss_cc : {loss_cc}')
            # print(f'loss_cg : {type(loss_cg)}')
            # print(f'loss_cg : {loss_cg}')
            # print('')
            # print('')
            # print(f'loss_info numpy transfo type : {type(loss_info.cpu().detach().numpy())}')

            self.loss_d.append(loss_d.detach().cpu())
            self.loss_g.append(loss_g.detach().cpu())
            self.conditional_loss.append(cross_entropy.detach().cpu())
            self.raw_loss_g.append(raw_loss_g.detach().cpu())
            self.loss_info.append(loss_info.detach().cpu())
            self.loss_cc.append(loss_cc.detach().cpu())
            self.loss_cg.append(loss_cg.detach().cpu())
            self.loss_total_g.append(loss_total_g.detach().cpu())

            # self.loss_d.append(loss_d.cpu().detach().numpy())
            # self.loss_g.append(loss_g.cpu().detach().numpy())
            # self.conditional_loss.append(cross_entropy.cpu().detach().numpy())
            # self.raw_loss_g.append(raw_loss_g.cpu().detach().numpy())
            # self.loss_info.append(loss_info.cpu().detach().numpy())
            # self.loss_cc.append(loss_cc.cpu().detach().numpy())
            # self.loss_cg.append(loss_cg.cpu().detach().numpy())
            # self.loss_total_g.append(loss_total_g.cpu().detach().numpy())
            

        # plot_history(d_loss=self.loss_d, g_loss=self.loss_g)
