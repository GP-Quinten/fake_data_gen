...
class DataPrep(object):
    ...
    6) pb_type -> dictionary of problem type (i.e classification/regression) and target column
    ...
    def __init__(
        ...
        try:
            target_col = list(pb_type.values())[0]
        except:
            target_col = []

        y_real = raw_df[target_col]
        X_real = raw_df.drop(columns=[target_col])
        try:  ## Issue with stratify argument that only applies to classification task
